			...
			client.request("workspace/_roslyn_restore", result, function(err, response)
				...
				if response then
					for _, v in ipairs(response) do
						vim.notify(v.message:sub(1, #v.message-1))--avoid "Premi INVIO o un comando per proseguire"
					end
				end
			end)
...
---@field filewatchingServer? boolean --true if the server should watch for file changes, false if they are watched by the client
---@field exe? string|string[]
---@field config vim.lsp.ClientConfig
---@field choose_sln? fun(solutions: string[]): string?
	...
	function M.setup(config)
		...
		vim.api.nvim_create_autocmd("BufWinEnter", {
			callback = function(env)
				utils.did_change_watched_file(vim.uri_from_bufnr(env.buf))
			end
		})
